...
static void RunBench(Bus& bus, const char* label, uint32_t count) {
  uint32_t received = 0;
  auto handle = bus.template Subscribe<MsgT>([&received](const osp::MessageEnvelope<BigPayload>&) { ++received; });
  ...
  double mbps = (ms > 0) ? (static_cast<double>(received) * sizeof(MsgT) / 1048576.0 / (ms / 1000.0)) : 0;
...
int main() {
  ...
  printf("QueueDepth=%u, BatchSize=%u, Envelope sizeof=%zu\n\n", static_cast<uint32_t>(OSP_BUS_QUEUE_DEPTH),
         static_cast<uint32_t>(OSP_BUS_BATCH_SIZE), sizeof(osp::MessageEnvelope<BigPayload>));
