...
using BusPayload = std::variant<PeerEvent, StartTestCmd, StopTestCmd, StatsSnapshot, EchoResult, FileProgress>;
...
inline bool VerifyPattern(const uint8_t* buf, uint32_t len, uint32_t seq) noexcept {
  if (buf == nullptr)
    return false;
  for (uint32_t i = 0; i < len; ++i) {
    if (buf[i] != static_cast<uint8_t>((seq + i) & 0xFFU))
      return false;
  }
  return true;
}
...
inline bool LoadConfig(const char* path, NetStressConfig& cfg) {
  ...
  // [client]
  cfg.client_num = static_cast<uint32_t>(ini.GetInt("client", "num_clients", static_cast<int32_t>(cfg.client_num)));
  cfg.client_interval_ms =
      static_cast<uint32_t>(ini.GetInt("client", "interval_ms", static_cast<int32_t>(cfg.client_interval_ms)));
  cfg.client_payload_len =
      static_cast<uint32_t>(ini.GetInt("client", "payload_len", static_cast<int32_t>(cfg.client_payload_len)));
  cfg.client_shell_port = ini.GetPort("client", "shell_port", cfg.client_shell_port);

  // [monitor]
  cfg.monitor_probe_ms =
      static_cast<uint32_t>(ini.GetInt("monitor", "probe_interval_ms", static_cast<int32_t>(cfg.monitor_probe_ms)));
  cfg.monitor_shell_port = ini.GetPort("monitor", "shell_port", cfg.monitor_shell_port);

  // [network]
  cfg.connect_timeout_ms =
      static_cast<uint32_t>(ini.GetInt("network", "connect_timeout_ms", static_cast<int32_t>(cfg.connect_timeout_ms)));
  cfg.max_clients = static_cast<uint32_t>(ini.GetInt("network", "max_clients", static_cast<int32_t>(cfg.max_clients)));
