...
static net_stress::HandshakeResp HandleHandshake(const net_stress::HandshakeReq& req, void* /*ctx*/) {
  net_stress::HandshakeResp resp{};
  resp.server_id = net_stress::kServerId;
  std::strncpy(resp.server_name, "net_stress_server", sizeof(resp.server_name) - 1);

  if (req.version != net_stress::kProtocolVersion) {
    resp.accepted = 0;
    OSP_LOG_WARN("SERVER", "Rejected client %u: bad version %u", req.client_id, req.version);
    return resp;
  }
  ...
  OSP_LOG_INFO("SERVER", "Client %u (%s) -> slot %u", req.client_id, req.name, slot);
  return resp;
}

static net_stress::EchoResp HandleEcho(const net_stress::EchoReq& req, void* /*ctx*/) {
  ...
  // Echo payload back
  uint32_t copy_len = (req.payload_len > net_stress::kMaxPayloadBytes) ? net_stress::kMaxPayloadBytes : req.payload_len;
...
static net_stress::FileTransferResp HandleFileTransfer(const net_stress::FileTransferReq& req, void* /*ctx*/) {
  net_stress::FileTransferResp resp{};
  resp.server_id = net_stress::kServerId;
  resp.chunk_seq = req.chunk_seq;

  uint32_t chunk_len = (req.chunk_len > net_stress::kMaxPayloadBytes) ? net_stress::kMaxPayloadBytes : req.chunk_len;

  // Simulate receiving file data (verify pattern)
  bool valid = net_stress::VerifyPattern(req.data, chunk_len, req.chunk_seq);
  if (!valid) {
    OSP_LOG_WARN("FILE", "Chunk %u from client %u: pattern mismatch", req.chunk_seq, req.client_id);
    resp.accepted = 0;
    return resp;
  }
  ...
  if (resp.complete != 0) {
    OSP_LOG_INFO("FILE", "File from client %u complete: %u chunks, %u bytes", req.client_id, req.total_chunks,
                 req.file_size);
  }
  return resp;
}
...
static void StatsTimerCb(void* /*ctx*/) {
  ...
  double throughput_kbps =
      (elapsed > 0) ? static_cast<double>(bytes + fbytes) * 8.0 / static_cast<double>(elapsed) : 0.0;

  OSP_LOG_INFO("STATS",
               "elapsed=%lums clients=%u echo=%u "
               "file_chunks=%u file_bytes=%lu throughput=%.1f kbps",
               static_cast<unsigned long>(elapsed), active, echo, fchunks, static_cast<unsigned long>(fbytes),
               throughput_kbps);
}
...
static int cmd_stats(int /*argc*/, char* /*argv*/[]) {
  ...
  if (elapsed > 0) {
    double kbps = static_cast<double>(bytes + fbytes) * 8.0 / static_cast<double>(elapsed);
...
int main(int argc, char* argv[]) {
  ...
  // Command-line positional overrides (skip --config pairs)
  {
    ...
    for (int i = 1; i < argc; ++i) {
      ...
      if (pos == 1)
        hs_port = static_cast<uint16_t>(std::atoi(argv[i]));
      else if (pos == 2)
        echo_port = static_cast<uint16_t>(std::atoi(argv[i]));
      else if (pos == 3)
        file_port = static_cast<uint16_t>(std::atoi(argv[i]));
      ++pos;
    }
  }

  OSP_LOG_INFO("SERVER", "=== Net Stress Server ===");
  OSP_LOG_INFO("SERVER", "Handshake: %u, Echo: %u, File: %u, Shell: %u", hs_port, echo_port, file_port, shell_port);

  InitSlots();
  g_start_time_ms = net_stress::NowMs();

  // --- Handshake Service ---
  osp::Service<net_stress::HandshakeReq, net_stress::HandshakeResp> hs_svc({hs_port, 8, 4});
  hs_svc.SetHandler(HandleHandshake);
  auto hs_r = hs_svc.Start();
  if (!hs_r.has_value()) {
    OSP_LOG_ERROR("SERVER", "Failed to start handshake service on port %u", hs_port);
    return 1;
  }
  ...
  // --- Echo Service ---
  osp::Service<net_stress::EchoReq, net_stress::EchoResp> echo_svc({echo_port, 8, net_stress::kMaxClients});
  echo_svc.SetHandler(HandleEcho);
  auto echo_r = echo_svc.Start();
  if (!echo_r.has_value()) {
    OSP_LOG_ERROR("SERVER", "Failed to start echo service on port %u", echo_port);
    return 1;
  }
  ...
  osp::Service<net_stress::FileTransferReq, net_stress::FileTransferResp> file_svc(
      {file_port, 8, net_stress::kMaxClients});
  file_svc.SetHandler(HandleFileTransfer);
  auto file_r = file_svc.Start();
  if (!file_r.has_value()) {
    OSP_LOG_ERROR("SERVER", "Failed to start file service on port %u", file_port);
    return 1;
  }
