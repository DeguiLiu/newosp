...
static void PrintStats(const char* label, uint32_t pub, uint32_t recv, uint32_t drop) {
  double drop_rate = (pub > 0) ? (100.0 * drop / pub) : 0.0;
  std::printf("  %-18s  pub=%-6u recv=%-6u drop=%-6u rate=%.1f%%\n", label, pub, recv, drop, drop_rate);
}
...
int main() {
  ...
  monitor.Subscribe<CriticalAlert>(
      [](const CriticalAlert&, const osp::MessageHeader&) { g_critical_recv.fetch_add(1, std::memory_order_relaxed); });
  monitor.Subscribe<TelemetryData>([](const TelemetryData&, const osp::MessageHeader&) {
    g_telemetry_recv.fetch_add(1, std::memory_order_relaxed);
  });
  monitor.Subscribe<DiagnosticLog>([](const DiagnosticLog&, const osp::MessageHeader&) {
  ...
  for (uint32_t i = 0; i < 100; ++i) {
    bus.PublishWithPriority(Payload{CriticalAlert{i, 1}}, 1, osp::MessagePriority::kHigh);
    g_critical_pub.fetch_add(1, std::memory_order_relaxed);

    bus.PublishWithPriority(Payload{TelemetryData{static_cast<uint16_t>(i), 1.0f * i, NowUs()}}, 1,
  ...
  OSP_LOG_INFO("priority", "Phase 2: Heavy load (10k LOW, 5k MED, 1k HIGH)");
  for (uint32_t i = 0; i < 10000; ++i) {
    TelemetryData td{static_cast<uint16_t>(i & 0xFFFF), 0.1f * i, NowUs()};
    if (!bus.PublishWithPriority(Payload{td}, 1, osp::MessagePriority::kLow)) {
  ...
  for (uint32_t i = 0; i < 5000; ++i) {
    ...
    if (!bus.PublishWithPriority(Payload{dl}, 1, osp::MessagePriority::kMedium)) {
  ...
  for (uint32_t i = 0; i < 1000; ++i) {
    ...
    if (!bus.PublishWithPriority(Payload{ca}, 1, osp::MessagePriority::kHigh)) {
  ...
  if (crit_drop_pct < 1.0) {
    OSP_LOG_INFO("priority", "PASS: CriticalAlert drop rate %.1f%% < 1%%", crit_drop_pct);
  } else {
    OSP_LOG_WARN("priority", "FAIL: CriticalAlert drop rate %.1f%% >= 1%%", crit_drop_pct);
  }
