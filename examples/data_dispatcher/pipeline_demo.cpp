...
static void PreprocessHandler(const uint8_t* data, uint32_t len, uint32_t /*block_id*/, void* ctx) {
...
static void LogStageHandler(const uint8_t* data, uint32_t len, uint32_t /*block_id*/, void* ctx) {
  ...
  if (log->frames_received % 10 == 0) {
    OSP_LOG_INFO("Logging", "frame #%u seq=%u pts=%u ts=%ums", log->frames_received, hdr->seq_num, hdr->point_count,
                 hdr->timestamp_ms);
  }
}
...
static void FusionStageHandler(const uint8_t* data, uint32_t len, uint32_t /*block_id*/, void* ctx) {
  ...
  auto* pts = reinterpret_cast<const LidarPoint*>(data + sizeof(LidarFrame));
  for (uint32_t i = 0; i < hdr->point_count; ++i) {
    if (pts[i].x < fus->min_x)
      fus->min_x = pts[i].x;
    if (pts[i].x > fus->max_x)
      fus->max_x = pts[i].x;
    if (pts[i].y < fus->min_y)
      fus->min_y = pts[i].y;
    if (pts[i].y > fus->max_y)
      fus->max_y = pts[i].y;
    if (pts[i].z < fus->min_z)
      fus->min_z = pts[i].z;
    if (pts[i].z > fus->max_z)
      fus->max_z = pts[i].z;
  }
...
int main(int argc, char* argv[]) {
  ...
  if (argc > 1)
    max_frames = static_cast<uint32_t>(std::atoi(argv[1]));

  OSP_LOG_INFO("Pipeline", "starting: frames=%u block_size=%u pool=%u", max_frames, kFrameDataSize, kPoolMaxBlocks);
  ...
  disp.SetBackpressureCallback(
      [](uint32_t free_count, void*) { OSP_LOG_WARN("Pipeline", "backpressure: free=%u", free_count); }, nullptr);

  osp::FaultReporter reporter;
  reporter.fn = [](uint16_t idx, uint32_t detail, osp::FaultPriority pri, void* ctx) {
    static_cast<osp::FaultCollector<4, 16>*>(ctx)->ReportFault(idx, detail, pri);
  };
  ...
  timer.Add(
      ...
      [](void* arg) {
        ...
        float fps = (dt > 0) ? static_cast<float>(s->log->frames_received) * 1e6f / static_cast<float>(dt) : 0.0f;
        OSP_LOG_INFO("Pipeline", "stats: pp=%u log=%u fus=%u gaps=%u free=%u alloc=%u fps=%.1f",
                     s->pp->frames_processed, s->log->frames_received, s->fus->frames_processed, s->log->seq_gaps,
                     s->disp->FreeBlocks(), s->disp->AllocBlocks(), static_cast<double>(fps));
      },
      &stats);
  ...
  timer.Add(
      ...
      [](void* arg) {
        ...
        if (t > 0)
          OSP_LOG_WARN("Pipeline", "ScanTimeout reclaimed %u blocks", t);
      },
      &disp);
  ...
  // --- ForceCleanup demo ---
  uint32_t cleaned = disp.ForceCleanup([](uint32_t, void*) -> bool { return true; }, nullptr);
  if (cleaned > 0)
    OSP_LOG_INFO("Pipeline", "ForceCleanup reclaimed %u blocks", cleaned);

  // --- Summary ---
  uint64_t dt = osp::SteadyNowUs() - t0;
  float fps = (dt > 0) ? static_cast<float>(frames_sent) * 1e6f / static_cast<float>(dt) : 0.0f;
  OSP_LOG_INFO("Pipeline", "=== Summary ===");
  OSP_LOG_INFO("Pipeline", "  Produced:   %u (alloc_fail=%u)", frames_sent, alloc_failures);
  OSP_LOG_INFO("Pipeline", "  Preprocess: %u (filtered_pts=%u)", pp_ctx.frames_processed, pp_ctx.points_filtered);
  ...
  if (fus_ctx.frames_processed > 0) {
    OSP_LOG_INFO("Pipeline", "  BBox: x=[%.1f,%.1f] y=[%.1f,%.1f] z=[%.1f,%.1f]", static_cast<double>(fus_ctx.min_x),
                 static_cast<double>(fus_ctx.max_x), static_cast<double>(fus_ctx.min_y),
                 static_cast<double>(fus_ctx.max_y), static_cast<double>(fus_ctx.min_z),
