...
static int shm_status(int /*argc*/, char* /*argv*/[]) {
  ...
  double avg = (polls > 0) ? static_cast<double>(g_state.total_depth_sum.load(std::memory_order_relaxed)) / polls : 0.0;
  ...
  osp::DebugShell::Printf(
      ...
      "  min/max:    %u / %u\r\n"
      "  polls:      %u\r\n",
      g_state.channel_name, g_state.channel_open ? "yes" : "no", depth, kSlotCount, avg,
      g_state.min_depth.load(std::memory_order_relaxed), g_state.max_depth.load(std::memory_order_relaxed), polls);
  return 0;
}
...
static int shm_stats(int /*argc*/, char* /*argv*/[]) {
  ...
  uint32_t polls = g_state.poll_count.load(std::memory_order_relaxed);
  double avg = (polls > 0) ? static_cast<double>(g_state.total_depth_sum.load(std::memory_order_relaxed)) / polls : 0.0;
  double fill_pct = (kSlotCount > 0) ? (static_cast<double>(depth) / kSlotCount * 100.0) : 0.0;
  ...
  osp::DebugShell::Printf(
      ...
      "  slot count:   %u\r\n"
      "  capacity:     %u KB\r\n",
      fill_pct, depth, kSlotCount, avg, g_state.min_depth.load(std::memory_order_relaxed),
      g_state.max_depth.load(std::memory_order_relaxed), polls, kSlotSize, kSlotCount, (kSlotSize * kSlotCount) / 1024);
  return 0;
}
...
static int shm_latest(int /*argc*/, char* /*argv*/[]) {
  ...
  if (g_state.has_producer_stats.load(std::memory_order_relaxed)) {
    ...
    osp::DebugShell::Printf("  [producer] frames=%u dropped=%u fps=%.1f mbps=%.1f depth=%u\r\n", p.frames_ok, p.dropped,
                            static_cast<double>(p.fps), static_cast<double>(p.mbps), p.queue_depth);
  } else {
  ...
  if (g_state.has_consumer_stats.load(std::memory_order_relaxed)) {
    ...
    osp::DebugShell::Printf(
        ...
        "fps=%.1f mbps=%.1f\r\n",
        c.frames_ok, c.frames_bad, c.gaps, c.stall_count, static_cast<double>(c.fps), static_cast<double>(c.mbps));
  } else {
...
static int shm_peek(int /*argc*/, char* /*argv*/[]) {
  ...
  if (size >= sizeof(FrameHeader)) {
    ...
    osp::DebugShell::Printf(
        ...
        hdr->magic, (hdr->magic == kMagic) ? "(OK)" : "(BAD)", hdr->seq_num, hdr->width, hdr->height, size);
  } else {
...
static int shm_config(int /*argc*/, char* /*argv*/[]) {
  osp::DebugShell::Printf(
      ...
      g_state.channel_name, kSlotSize, kSlotCount, (kSlotSize * kSlotCount) / 1024, kMagic, kFrameSize, kWidth, kHeight,
      kStatsRingSize);
  return 0;
}
...
int main(int argc, char* argv[]) {
  ...
  uint16_t shell_port = (argc > 2) ? static_cast<uint16_t>(std::atoi(argv[2])) : 9527;
  ...
  if (use_console) {
    ...
    OSP_LOG_INFO("monitor", "shell started on port %u (telnet localhost %u)", shell_port, shell_port);
  }
  OSP_LOG_INFO("monitor",
               "commands: shm_status, shm_stats, shm_latest, "
  ...
  while (!shutdown.IsShutdownRequested()) {
    if (g_state.channel_open) {
      ...
      uint32_t count = g_state.poll_count.fetch_add(1, std::memory_order_relaxed) + 1;
      ...
      while (depth > cur_max && !g_state.max_depth.compare_exchange_weak(cur_max, depth, std::memory_order_relaxed)) {}

      uint32_t cur_min = g_state.min_depth.load(std::memory_order_relaxed);
      while (depth < cur_min && !g_state.min_depth.compare_exchange_weak(cur_min, depth, std::memory_order_relaxed)) {}
      ...
      // Periodic log
      if (count % 50 == 0) {
        double avg = static_cast<double>(g_state.total_depth_sum.load(std::memory_order_relaxed)) / count;
        OSP_LOG_INFO("monitor", "depth=%u/%u avg=%.1f min=%u max=%u polls=%u", depth, kSlotCount, avg,
                     g_state.min_depth.load(std::memory_order_relaxed),
                     g_state.max_depth.load(std::memory_order_relaxed), count);
      }
    }
